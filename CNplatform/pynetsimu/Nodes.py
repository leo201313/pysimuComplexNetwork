...
class Node(object):
    ...
    def ini(self,graph,load_lst):
        ...
        for nodeid in self.id_list:
            ...
            load_capacity = int(abs(random.normalvariate(100,20)) + 1.5*self.load[nodeid])
            self.load_capacity[nodeid] = load_capacity

            attack_cost = 10 + abs(random.normalvariate(20,5)) + 200 * (degree_centrality[nodeid] - degree_centrality[min_degree_node]) / degree_delta
